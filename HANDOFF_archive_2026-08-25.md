...
## 2026-08-25: the fourth-cell control is clean, and the swing is not the
static ellipticity defect

The control queued last session landed:
`flows/pre_chartsync/centroid_gauss2_deep.eqx` at 20k targets gives windowed,
corrected **m1 = -0.00851 +/- 0.00347**, against its 200k counterpart's
-0.01005 +/- 0.00170 -- well within errors.  The 2x2 is confirmed clean; the
post-fix centroid retrain (`flows/centroid_gauss2_deep.eqx`, +0.01530 +/-
0.00284) is the sole thing that differs.

### The 4.36x ellipticity-response defect is IDENTICAL pre- and post-fix

`centroid.py check` on both flows (`--copies copies_gauss2_deep.fits`):

|  | Mf shift | Mr shift | Mc shift | ellipticity response |
|---|---|---|---|---|
| catalog | -5.922e-03 | -1.131e-02 | -1.592e-02 | +8.3649e-03 |
| post-fix layer | -5.093e-03 | -8.244e-03 | -1.121e-02 | +3.6452e-02 (4.36x) |
| pre-fix layer | -5.668e-03 | -9.851e-03 | -1.349e-02 | +3.6311e-02 (4.35x) |

This is the layer's known Sigma_X-only (g=0) defect, flagged since 2026-08-24
and untouched by the chart fix -- confirmed here to be untouched by the
*centroid retrain* either: it is 0.4% different between two independent
trainings under different frozen shear layers.  **It is not the cause of the
-0.010 -> +0.0153 swing.**  (Note: it is real and still worth fixing, but as a
population-level correction to the shear-independent shape amplification, not
as the source of the estimator disagreement.)

### The swing is INSIDE the window, not the selection terms or the edge

Re-ran the post-fix cell standalone
(`logs_deep_cent_postfix.txt`, same command as the pre-fix control) to get its
quintile table.  `P_s`, `Q_s` and the window-crossing counts are essentially
identical between the two runs (P_s = 0.6324 both; Q_s matches to 3 sig figs;
12685/12672 targets cross the window both times) -- but the **windowed,
uncorrected** m1 itself jumped:

|  | windowed, uncorrected | windowed, corrected | unwindowed |
|---|---|---|---|
| pre-fix | +0.00802 +/- 0.00262 | -0.00851 +/- 0.00347 | -0.02518 +/- 0.00915 |
| post-fix | +0.03264 +/- 0.00183 | +0.01530 +/- 0.00284 | -0.02184 +/- 0.00243 |

The selection correction moves both by about the same additive amount
(~-0.017); the +0.025 swing is already there in the raw windowed estimate,
before any selection-PQR term touches it.  Since the window is Mr/Mf in
(2.2, 3.2) -- inside the resolution edge, not at it -- this rules out the
"support-edge leak" family of explanations too.

Unwindowed quintiles by Mf (post-fix, `logs_deep_cent_postfix.txt`, vs
pre-fix from `logs_deep_cent_pre_chartsync.txt`):

| Mf quintile | pre-fix m1 | post-fix m1 |
|---|---|---|
| q1 | -0.3733 +/- 0.0706 | -0.2967 +/- 0.0145 |
| q2 | -0.0072 +/- 0.0461 | -0.0366 +/- 0.0073 |
| q3 | +0.0222 +/- 0.0071 | +0.0239 +/- 0.0036 |
| q4 | +0.0119 +/- 0.0027 | +0.0148 +/- 0.0023 |
| q5 | +0.0014 +/- 0.0020 | -0.0000 +/- 0.0018 |

The point estimates track each other reasonably (same sign, same shape by
quintile); what changed is that the **post-fix bootstrap errors are 4-6x
tighter** in q1/q2 despite identical ESS (638 vs 637 median, frac<10 = 0.000
both).  Read together with the flat ellipticity check, this says the post-fix
run's per-object Q/R are simply better BEHAVED (lower per-object variance) --
consistent with the pre-fix run's -0.0085 being a noisier estimate of roughly
the same underlying quantity, not evidence the swing lives in the tail.

### Where this points: the g-conditioned slope, not the base response

`centroid.py check` only ever evaluates the layer at g = 0 (`_Coeffs.__call__`
defaults p1 = p2 = 0), so it cannot see the `s1`, `s2` slope terms
(`c56057e`, `dc41c8c`) that make the layer's response depend on shear at all.
Those terms only enter `bias.py`'s Q, R through the `jacfwd`/`hessian`
autodiff of the FULL chain's log-density w.r.t. g -- and that chain includes
the (frozen, but density-relevant) shear layer immediately downstream, which
went from ~4-5% wrong varying with Mr/Mf to accurate in the same fix.  A
cheap forward-only probe (`/tmp/.../probe_centroid_slopes.py`, not committed)
comparing `layer.coeffs(f,a,b,q)` against `layer.coeffs(f,a,b,q,p1,p2)` at
g=(0.02, 0) on 4000 real galaxies shows both flows' slope-driven `d(coeff)`
similarly saturated in magnitude (max |dcoeff| ~0.095-0.11 in both, close to
`_SLOPE_MAX`'s bound), so the retrain is not simply "one honest, one not" at
the coefficient level -- the two trainings converged to different slope
SHAPES (per-component means differ, sometimes in sign) under different frozen
downstream physics, and that is consistent with the whole swing being
concentrated in the g-dependent part of the layer rather than its static
part.

### Ablation confirms it: zeroing the slope flips the sign

Ran the decisive test.  Monkeypatched `models.centroid.g_invariants` to
return `(0, 0)` regardless of `g` -- this forces `_Coeffs.__call__` to use
only its zero-`p1`/`p2` default, i.e. every galaxy sees the layer's BASE
(Sigma_X-only) response and none of the trained `s1`,`s2` slope -- and reran
`bias.py` on `flows/centroid_gauss2_deep.eqx` at 4000 targets, same seed,
same window, against an unpatched run at the same 4000 targets:

|  | windowed, uncorrected | windowed, corrected |
|---|---|---|
| slope ON (as trained) | +0.03070 +/- 0.00391 | +0.01420 +/- 0.00652 |
| slope OFF (p1, p2 forced to 0) | -0.00746 +/- 0.00290 | -0.02277 +/- 0.00578 |

The slope-on number reproduces the 20k run's +0.0327 windowed-uncorrected
closely, confirming 4000 targets is enough to see the effect.  Zeroing the
slope **flips the sign** and moves windowed-uncorrected m1 by 0.038 -- this
is not a secondary contributor, it is the whole swing.  Script:
`/tmp/.../scratchpad/bias_noslope.py` (not committed; the monkeypatch is a
three-line wrapper around `bias.main()`, easy to reproduce).

### Conclusion and next step

The post-fix centroid retrain's `s1`/`s2` slope coefficients (the layer's
dependence on `p1 = Re(ebar g)`, `p2 = |g|^2`, added in `c56057e`) are the
entire source of the -0.010 -> +0.0153 swing.  They were trained under a
frozen shear layer that changed from ~4-5% wrong (varying with Mr/Mf) to
accurate between the pre- and post-fix runs, and NLL -- which cannot see the
physical sign constraint that already had to be hand-imposed on the BASE
coefficients (`_Coeffs.__call__`'s `t0_mask`/`e_mask`, `dc41c8c`) -- has no
such constraint on `s1`/`s2` at all.  The base coefficients needed a
structural fix because plain NLL preferred the wrong sign; the slope
coefficients were never checked for the analogous problem.

**Next session:** derive what sign/magnitude constraint the slope terms
physically owe (the g-conditioned shift is `E[marginalisation | m, g]`, and
marginalising a lensed galaxy should still cost flux/size/concentration
monotonically in `g`-space near `g = 0` -- work out whether that fixes a
sign the way the base terms' did, or whether it's a magnitude problem, before
touching `centroid.py train`).  Until then, every `flows/centroid_*.eqx`
built after `c56057e` should be treated as suspect for its g-dependent part
specifically, not just void from the chart bug.

### The slope's sign question, worked out: half already closed, half has no clean answer

Re-read `_Coeffs.__call__` (`models/centroid.py:375-439`) with the question in
mind.  `coeff = base + s1*p1 + s2*p2` is formed FIRST; only then does
`pos = +rational_bound(softplus(coeff))` / `neg = -rational_bound(softplus(coeff))`
get masked onto the t0-column (idx 0,2,4) and e-column (idx 1,3,5).  `softplus`
is >= 0 for *any* argument, so `pos >= 0` and `neg <= 0` hold **at every g**, not
just g = 0 -- the slope perturbs `coeff` before the softplus, never after.  That
closes half the question outright: the base terms' sign constraint (t0's
coefficient >= 0, e's <= 0, `dc41c8c`) already extends through `s1`, `s2`
automatically, by construction, no code change needed.  The physical fact
behind it -- displacing the origin costs Mf/Mr/Mc-inverse for every copy of
every galaxy, and (ec*t2).real <= 0 for isotropic Sigma_X -- is a deterministic
per-galaxy geometric fact (t0 is half the trace of a PSD matrix; t2's phase
locks to e), true regardless of shear, so it was never at risk from the slope
in the first place.

What is *not* protected: how strongly `softplus(coeff)`'s **magnitude** moves
with p1, p2 (bounded only by `_SLOPE_MAX = 0.1` on s1, s2 individually -- no
constraint ties its sign or rate to anything physical), and the three spin-2
coefficients A, B, D (idx 6,7,8) that build `de_raw` directly -- these get no
sign mask at all, base or slope, and are exactly what sets the
ellipticity-response the whole investigation started from.

For s1/s2 themselves, I don't think a universal sign exists to derive, and
manufacturing one would risk repeating the t0 mistake (a hand-imposed
constraint that turned out backwards, 2026-08-24).  The base terms' sign is a
statement about ONE galaxy's geometry (true for every template, every copy,
every origin displacement, independent of shear).  s1/s2 instead answer
`E[marginalisation | m, g] - E[marginalisation | m, 0]`: whether the AVERAGE
marginalisation strength differs between the population of profiles that land
at a given observed m under shear g vs. under no shear -- a statement about
which profiles occupy a given m, i.e. a population-mixture/selection effect,
not a per-object geometric fact.  Nothing about J or Sigma_u alone pins its
sign, and it is free to differ between (f,a,b,q) cells depending on gauss2's
specific bulge+disc mix.

What DOES transfer from the base derivation is its *method*, not its
conclusion: the base sign was pinned by regressing the catalog's true
`marginalize(x)-x` against `t0`, `(ec*t2).real` at g = 0.  The same move is
available at g != 0 for free -- `copies_gauss2_deep.fits`'s per-copy
`dm_dg`/`d2m_dg2` (and `GALAXIES`' own) are exactly what `centroid.py train`
already lenses copies with, so `weighted_copy_mean` evaluated on copies lensed
by a few g's, regressed against `p1*t0`, `p1*(ec*t2).real` (and the spin-2
analogues feeding A, B, D), gives an EMPIRICAL sign/magnitude target instead of
a guessed one.

Caveat that stopped me from just running it this session: what `bias.py`'s
`jacfwd` actually sees is `d(unmarginalize)/dg` **at fixed x** (the observed
moments held still, only the conditioned g varying) -- that isolates s1/s2
alone, which is what needs validating.  A finite difference built by lensing a
copy's own moments with g (mirroring what `train` does) instead computes
`d(shift)/dg` letting x move too, which conflates the slope with the
"`t0(z)` changes because z changes with g" path that's already present with
the slope OFF (and is presumably fine, since zeroing the slope alone flipped
the sign back to something closer to the pre-fix control).  A clean regression
needs binning by OBSERVED (post-lens) m across several g draws, not a
per-copy finite difference at ±g -- more work than a quick script, so it's the
concrete next action rather than something attempted tonight.

**Next step:** build the m-binned, multi-g regression above (extend
`centroid.check` or a standalone script) to get an empirical sign/magnitude
target for the slope, prioritising A/B/D first -- they are the coefficients
`softplus` does *not* protect, and they drive the ellipticity response
directly.  Do not hand-impose a sign mask on s1/s2 or A/B/D without that
empirical grounding first.
