...
# ONE JAX job at a time: block until the host-path probe has released the card.
# Wait on the host-path probe's SHELL pid, not on `bias.py`: there is a gap
# between its two runs where no bias.py exists and we would start into it.
while [ -d /proc/857692 ]; do sleep 20; done
...
one() {   # $1 = n-targets
    ...
    BFD_DEVICE_PQR=1 python -u bias.py \
        ...
        > "logs/ratedev_$n.log" 2>&1
    t1=$(date +%s)
    echo "$n $((t1 - t0))" | tee -a "$OUT/ratedev.txt"   # no bc on this box
}
...
print(f"\n  per integrated target (2 arms, DEVICE path): {slope*1e3:.2f} ms")
