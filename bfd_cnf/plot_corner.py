...
_DEFAULT_LOG_SCALE = {"train": 11.4, "draw": 11.4}
...
def resample(m, w, n, label):
    ...
    if w is None:
        sel = np.random.default_rng(0).choice(
            len(m), size=min(n, len(m)), replace=False
        )
        return m[sel]
...
def load_train(args):
    ...
    X = np.asarray(ds["centroid_moments_jnp"], dtype=np.float64)[
        :, :2
    ]  # [MX,MY] at g=0
...
def load_draw(args):
    ...
    w = np.asarray(
        ds["weights"], dtype=np.float64
    )  # ∝ nda·detj (batch-sampling proposal)
...
def sample_flow(prior_trained, raw2standard, key, n, log_scale):
    ...
    raw = np.asarray(
        jnp.clip(
            raw, jnp.array([1.0, 1e-3, -jnp.inf, -jnp.inf]), jnp.array([jnp.inf] * 4)
        )
    )
...
def draw_overlay(fig):
    ...
    for a in (10, 15):
        axs[a].axvline(-1.0, lw=1, c="tab:green")
        axs[a].axvline(1.0, lw=1, c="tab:green")
    ...
    axs[9].axvline(STELLAR, lw=1, color="tab:red")
...
def main():
    ...
    ap.add_argument(
        "--n-plot", type=int, default=1_000_000, help="points per distribution"
    )
    ap.add_argument(
        "--prior", default=None, help="prior flow .eqx (default: canonical)"
    )
    ...
    ap.add_argument(
        "--no-flow", action="store_true", help="data cloud only, no flow overlay."
    )
    ap.add_argument(
        "--no-overlay",
        action="store_true",
